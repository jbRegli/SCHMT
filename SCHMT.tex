...
\renewcommand{\abstractname}{Abstract:}
  \begin{abstract}
    A scattering convolutional hidden Markov tree proposes a new method to perform inference on an high-dimensional signal. It combines the interesting signal representation created by the scattering transform to a powerful probabilistic graphical model.  
    %It expresses the signal representation created by a scattering convolutional network as a hidden Markov tree.
    A wavelet scattering network computes a signal representation which is translation invariant, stable to deformations while preserving the informative content of the signal by cascading wavelet transform convolutions with nonlinear modulus and averaging operators. The network's structure and its distribution are described using a hidden Markov tree. This tree offers a mean of performing several inference tasks among which are predictions. The scattering convolutional hidden Markov tree displays promising results on both classification and segmentation tasks of complex images.
  \end{abstract}
      ...
      Properties of a ``good'' representation for prediction can be intuited by looking how humans handle the classification/labeling of visual stimulus and what the elements ensuring good generalization capacities are. Using this approach, one can intuit the following properties:\\
			...
			\begin{itemize}
				...
				\begin{figure}[h]
					...
					\caption[Stability to deformations.]{\centering Stability to deformations: Humans can easily tell that (a) and (b) are from the same class. (c) can still be recognized even though it is slightly more challenging. For (d) the informative content is lost.}
...
% 		
  \section{Sonar Imagery:}
		...
		\begin{figure}[h]
			\centering
			\includegraphics[height=3.0in]{seg_original.eps}

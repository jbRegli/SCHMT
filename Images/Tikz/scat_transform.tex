...
\hspace*{-35mm}%
    ...
    level 2/.style={sibling distance=12mm, level distance = 2cm}
		...
		\node [arn_U][label=above:{$x$}][xshift=-10mm, yshift=+0mm] {$U_{\emptyset}$}
			% S node:
			...
			child{node [arn_U][label={[label distance=00mm]93:{\color{gray}$U[\lambda_{1}]x = |x \ast \psi_{\lambda_{1}}|$}}]{$U_{1}$}  
						...
						% Propagation nodes:
						child{node [arn_U][label={[label distance=00mm]93:{\color{gray}$U[\lambda_{1},\lambda_{2}]x = ||x \ast \psi_{\lambda_{1}}| \ast \psi_{\lambda_{2}} |$}}] {$U_{1,1}$}
							% S node:
